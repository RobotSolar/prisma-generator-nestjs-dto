...
export function parseApiProperty(
  ...
  const optional = Boolean(field.isNullable || !field.isRequired);

  properties.push({ name: 'nullable', value: optional ? 'true' : 'false' });
  properties.push({ name: 'required', value: optional ? 'false' : 'true' });
